...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.test_button1 = customtkinter.CTkButton(
            self.f1, text="CFG GENERATOR", command=self.runcfggenerator, font=('Times New Roman', 17, 'bold'))
        self.test_button2 = customtkinter.CTkButton(
            self.f1, text="CFG VALIDATOR", command=self.runcfgvalidator, font=('Times New Roman', 17, 'bold'))
    ...
    def runcfgvalidator(self):
        """
        runcfgvalidator runs cfg_validator.py
        """
        self.output_text.configure(state="normal")
        self.output_text.delete("1.0", "end")
        ...
        text_file.close()
        exec(open(os.path.abspath('Sources/cfg_validator.py')).read())
        self.output_text.configure(state="disabled")

    def runcfggenerator(self):
        """
        runcfggenerator runs cfg_generator.py
        """
        self.output_text.configure(state="normal")
        self.output_text.delete("1.0", "end")
        text_file = open(os.path.abspath('Files/input.txt'), "w")
        ...
        text_file.write(self.input_string.get(1.0, customtkinter.END))
        text_file.close()
        exec(open(os.path.abspath('Sources/cfg_generator.py')).read())
        self.output_text.configure(state="disabled")
